from odoo import models, api, http, fields, _
...
class BarcodePatch(http.Controller):
    ...
    def scan_from_main_menu(self, barcode):
        ...
        _logger.info("scan_from_main_menu")
        ...
        if request.env.user.has_group('stock.group_stock_multi_locations') and \
           (not barcode_type or barcode_type in ['location', 'dest_location']):
            ret_open_location = self._try_open_location(barcode)
    ...
    def _try_open_location(self, barcode):
        """ If barcode represents a location, open a list view to show all
        the quants in this location.
        """
